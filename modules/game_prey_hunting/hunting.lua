...
local function closeHuntingMessageWindow()
...
function init()
	...
	connect(g_game, {
		...
		onTaskHuntingTimeLeft = onUpdateRerrolTime,
		...
		onTaskHuntingBasicData = onPreyHuntingBaseData,
		onHuntingListSelectState = onHuntingListSelectState,
		onHuntingExhaustedState = onHuntingExhaustedState,
		onPreySelection = onPreySelection,
		onHuntingCompletedState = onHuntingCompletedState,
	})
	-- Handler compatível com servidores OTC padrão
	...
	for i = 1, 3 do
		...
		-- Configura estado inicial como exhausted
		slot:recursiveGetChildById('title'):setText("Exhausted")

		-- Desabilita botões do exhaust por padrão
		local rerollButton = exhaustWindow:recursiveGetChildById('exhaustRerollButton')
		if rerollButton then
			rerollButton:setEnabled(false)
			rerollButton:setOn(false)
		end

		local selectButton = exhaustWindow:recursiveGetChildById('exhaustSpecificHunting')
		if selectButton then
			selectButton:setEnabled(false)
			selectButton:setOn(false)
		end

		local chooseButton = exhaustWindow:recursiveGetChildById('chooseTaskButton')
...
function onPreySelection(slot, names, outfits, timeUntilFreeReroll, wildcards)
	-- Chama o handler já existente, adaptando os parâmetros
	onHuntingSelectState(slot, 2, nil, names, nil, timeUntilFreeReroll)
end
...
function show(position)
	...
	onResourceBalance(localPlayer:getResourceBalance(ResourceTypes.TASK_HUNTING), nil, ResourceTypes.TASK_HUNTING)
...
function sendHuntingMessageBox(slot, type)
	...
	if type == "cancel" then
		local yesFunction = function()
			...
			g_game.taskHuntingAction(slot, PREY_HUNTING_ACTION_REMOVE, false, 0)
			huntingWindow:show()
			closeHuntingMessageWindow()
			-- g_client.setInputLockWidget(huntingWindow)
		end
	...
	elseif type == "upgrade" or type == "wildcard" then
		...
		local yesFunction = function()
			g_game.taskHuntingAction(slot, action, false, 0)
			huntingWindow:show()
			closeHuntingMessageWindow()
			-- g_client.setInputLockWidget(huntingWindow)
		end
	...
	elseif type == "reroll" then
		local yesFunction = function()
			g_game.taskHuntingAction(slot, PREY_HUNTING_ACTION_LISTREROLL, false, 0)
			huntingWindow:show()
			closeHuntingMessageWindow()
			-- g_client.setInputLockWidget(huntingWindow)
		end
...
function selectWildcardCreature(slot)
	...
	g_game.taskHuntingAction(slot - 1, PREY_HUNTING_ACTION_SELECT, bestiaryUnlocked, raceID)
end
...
function onWildcardHuntingChange(panel, selected)
	...
	local creature = g_things.getRaceData(tonumber(selected:getId()))
	panel.window:recursiveGetChildById('title'):setText(short_text(tr("Selected: %s", string.capitalize(creature.name)),
		28))
	panel.selection.panel.creature:setOutfit(creature.outfit)
...
function onResourceBalance(balance, oldBalance, resourceType)
	...
	elseif resourceType == ResourceTypes.TASK_HUNTING then -- hunting tokens
		huntingToken = balance
	...
	-- setStringColor(moneyTooltip, " $", "#f7e6fe")
	local moneyTooltip = invetoryMoney + bankBalance
...
function onHuntingSelectState(slot, state, _, creatureList, unlock, nextFreeReroll)
	local panel = huntingSlots[slot + 1]
	if not panel then
		return
	end
	...
	-- creatureList is now a map: { [raceId] = bestiaryUnlocked }
	...
	-- creatureList is now a map: { [raceId] = bestiaryUnlocked }
	for raceId, bestiaryUnlocked in pairs(creatureList) do
		local box = g_ui.createWidget("PreyHuntingCreatureBox", monsterList)
		local monster = g_things.getRaceData(raceId) or 0
		if monster and (monster.name and monster.name ~= "") then
			box.creature:setOutfit(monster.outfit)
			box.creature:setRaceID(raceId)
			box:setActionId(slot + 1)
			box.creature:setTooltip(string.capitalize(monster.name))
			...
			if not firstSelected and not inactiveSelections[slot + 1] then
				...
				panel.window:recursiveGetChildById('title'):setText(short_text(
					tr("Selected: %s", string.capitalize(monster.name)), 28))
			end
		end
		...
		panel.inactive.choose.chooseTaskButton.onClick = function()
			...
			if selection and selection:isChecked() and selection.creature then
				...
				return g_game.taskHuntingAction(slot, PREY_HUNTING_ACTION_SELECT, bestiaryUnlocked,
	...
	onUpdateRerrolTime(slot, nextFreeReroll)
end

function onHuntingActiveState(slot, state, currentMonster, unlocked, toKill, killed, stars)
	local panel = huntingSlots[slot + 1]
	if not panel then
		return
	end
	...
	if completed then
		...
		panel.active.pick.pickReward.onClick = function()
			g_game.taskHuntingAction(slot, PREY_HUNTING_ACTION_COLLECT,
				false, 0)
		end
	else
	...
	local monster = g_things.getRaceData(currentMonster)
	creatureWidget:setOutfit(monster.outfit)
	creatureWidget:setCenter(true)
	creatureWidget:setActionId(slot + 1)

	local titleLabel = panel.window:recursiveGetChildById('title')
	titleLabel:setText(short_text(tr("Currently hunting: %s", string.capitalize(monster.name)), 29))
	titleLabel:setTextAutoResize(true)

	selectedMonster[slot + 1] = {
		currentRace = monster.name,
		maxKills = toKill,
		currentKills = killed,
		reward =
			rewardCount,
	...
	activeMonsterList[slot + 1] = monster.name
	...
	panel.active.monsterName = monster.name
	panel.active.stars = stars
	panel.active.rewardCount = rewardCount
	updatePreyWidget(slot, state, monster.outfit)
end

function onHuntingListSelectState(slot, state, _, monsterList, nextFreeReroll)
	local panel = huntingSlots[slot + 1]
	if not panel then
		return
	end
	...
	-- monsterList is now a map: { [raceId] = bestiaryUnlocked }
	...
	for k, v in pairs(monsterList) do
		...
		local creature = g_things.getRaceData(k)
		if creature then
...
function onHuntingExhaustedState(slot, state)
	...
	-- Desabilitar o botão de reroll
	local rerollButton = panel.exhaust:recursiveGetChildById('exhaustRerollButton')
	rerollButton:setActionId(slot)
	rerollButton:setEnabled(false)
	rerollButton:setOn(false)

	-- Desabilitar o botão de selecionar monstro wildcard
	local selectButton = panel.exhaust:recursiveGetChildById('exhaustSpecificHunting')
	selectButton:setActionId(slot)
	selectButton:setEnabled(false)
	selectButton:setOn(false)

	-- Desabilitar o botão de escolher task
	local chooseButton = panel.exhaust:recursiveGetChildById('chooseTaskButton')
	chooseButton:setEnabled(false)
	chooseButton:setOn(false)

	-- Configurar tempo de reroll (desabilitado mas mostra o tempo)
	...
	-- Atualizar preços (desabilitados - cor cinza)
	panel.exhaust.exhaustReroll.price.text:setText(convertLongGold(goldUpdatePrice, true))
	panel.exhaust.exhaustReroll.price.text:setColor("#707070")

	panel.exhaust.exhaustSelect.price.text:setText(wildcardSelectPrice)
	panel.exhaust.exhaustSelect.price.text:setColor("#707070")

	-- Configurar action IDs para hover
...
function updateVisibleRerollTime()
	...
	for slot, data in pairs(nextRerollTime) do
		...
		local timeLeft = math.max(0, data.timeLeft - elapsedSeconds)
		-- Corrige bug do 1min eterno: força atualização mesmo se travar em 60s
		if timeLeft <= 60 and timeLeft > 0 then
			-- força decremento manual se travar
			timeLeft = timeLeft - 1
			nextRerollTime[slot].timeLeft = timeLeft
			nextRerollTime[slot].startTime = currentTime
		end
		setTimeUntilFreeReroll(slot, timeLeft)
	end
end
...
function onHuntingCompletedState(slot, state, raceId, upgraded, toKill, killed, stars, nextFreeReroll)
	local panel = huntingSlots[slot + 1]
	if not panel then
		return
	end
	...
	-- Desabilitar o botão de cancel
	local cancelButton = panel.active:recursiveGetChildById('removeButton')
	cancelButton:setEnabled(false)
	cancelButton:setOn(false)

	-- Desabilitar o botão de upgrade de reward
	local upgradeButton = panel.active:recursiveGetChildById('pickHigherReward')
	upgradeButton:setEnabled(false)
	upgradeButton:setOn(false)

	-- Habilitar o botão de "pick reward"
	panel.active.pick.inactivePickReward:setVisible(false)
	panel.active.pick.pickReward:setVisible(true)
	panel.active.pick.pickReward:setActionId(slot + 1)
	panel.active.pick.pickReward.onClick = function()
		g_game.taskHuntingAction(slot, PREY_HUNTING_ACTION_COLLECT, false, 0)
	end

	-- Atualiza informações visuais do monstro
	local monster = g_things.getRaceData(raceId)
	if monster then
		local creatureWidget = panel.active:recursiveGetChildById('creature')
		creatureWidget:setOutfit(monster.outfit)
		creatureWidget:setCenter(true)
		...
		local titleLabel = panel.window:recursiveGetChildById('title')
		titleLabel:setText(short_text(tr("Completed: %s", string.capitalize(monster.name)), 29))
		titleLabel:setTextAutoResize(true)

		activeMonsterList[slot + 1] = monster.name
	end

	-- Atualiza contador de kills (completo)
	local counter = panel.active:recursiveGetChildById('killCounter')
	counter:setPercent(100)
	counter:setText(tr("%s / %s", killed, toKill))

	-- Atualiza recompensa
	local rewardCount = getHuntingRewardPoints(raceId, stars, upgraded)
	local reward = panel.active:recursiveGetChildById('rewardPoints')
	reward:setText(rewardCount)
	...
	-- Atualiza preços dos botões (desabilitados mas ainda visíveis)
	panel.active.removeCreature.price.text:setText(formatMoney(goldRemovePrice, ","))
	panel.active.higherReward.price.text:setText(rerollWildcardPrice)

	-- Guarda informações do monstro selecionado
	rewardGrade[slot + 1] = stars
	selectedMonster[slot + 1] = {
		currentRace = monster and monster.name or "",
		maxKills = toKill,
		currentKills = killed,
		reward = rewardCount,
		grade = stars
