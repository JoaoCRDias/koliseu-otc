...
function Fusion:init()
	...
	fusionButtonProcced.onClick = function()
		...
		g_game.sendForgeAction(0, convergenceWidget:isChecked(), self.leftItemId, self.fusionTier, self.rightItemId,
			self.improveClicked, self.tierLossClicked)
	end
	...
	-- Disable dragging on all item widgets
	previewFusionItem:setDraggable(false)
	previewFusionItem2:setDraggable(false)
	fusionItem1:setDraggable(false)
	fusionItem2:setDraggable(false)
	...
	improveButton.onClick = function(widget)
		local baseChance = self.baseChance or 0
		local improvedChance = self.improvedChance or 0
		if Fusion.improveClicked then
			successRateValue:setColor("#F75F5F")
			successRateValue:setText(baseChance .. "%")
			...
			if Forge:getResourceBalance('core') >= 1 then
				...
				successRateValue:setColor("#FFFFFF")
			end
		else
			successRateValue:setColor('#44AD25')
			successRateValue:setText((baseChance + improvedChance) .. "%")
	...
	for i, itemsTable in ipairs(data) do
		for z, v in ipairs(itemsTable) do
			...
			itemWidget.onClick = function()
				self:clearActivePanelsForParent(fusionContent2:getId())
				fusionContent2:destroyChildren()
				fusionButtonProcced:setEnabled(false)
...
function Fusion:clearActivePanels()
	if self.activePanels then
		self.activePanels = {}
	end
	if self.activePanelList then
		self.activePanelList = {}
	end
end

function Fusion:clearItems()
	self:clearActivePanels()
	if self.contentPanel then
		self.contentPanel:destroyChildren()
	end
	if fusionContent2 then
		fusionContent2:destroyChildren()
	end
end

function Fusion:clearActivePanelsForParent(parentId)
	if not self.activePanels then return end
	local keysToRemove = {}
	for key, _ in pairs(self.activePanels) do
		if string.find(key, parentId) then
			table.insert(keysToRemove, key)
		end
	end
	for _, key in ipairs(keysToRemove) do
		self.activePanels[key] = nil
		if self.activePanelList then
			self.activePanelList[key] = nil
		end
	end
end
...
function Fusion:displayItems(preview)
	...
	scheduleEvent(function()
		...
		improveButton:setEnabled(true)
		successRateValue:setColor("#FFFFFF")
		successRateValue:setText((self.baseChance or 0) .. "%")
		Fusion.improveClicked = nil
