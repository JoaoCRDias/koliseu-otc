...
function Transfer:createButton()
	...
	self.mainButton.onClick = function(widget, mousePos, mouseButton)
		...
		self.widgetStorage.requiredTransferCoreItem:setItemId(Forge.itemIds.EXALTED_CORE)
...
function Transfer:init()
	...
	-- Disable dragging on all preview item widgets
	firstButtonPreviewItem:setDraggable(false)
	secondButtonPreviewItem:setDraggable(false)
	previewTransferItem:setDraggable(false)
...
function Transfer:clearActivePanels()
...
function Transfer:clearItems()
	self:clearActivePanels()
	if self.widgetStorage and self.widgetStorage.firstItemPanel then
		self.widgetStorage.firstItemPanel:destroyChildren()
	end
	if self.widgetStorage and self.widgetStorage.secondItemPanel then
		self.widgetStorage.secondItemPanel:destroyChildren()
	end
end

function Transfer:clearActivePanelsForParent(parentId)
	...
	for i, v in ipairs(data) do
		for z, donor in ipairs(v.donors) do
			...
			itemWidget.onClick = function()
				self:clearActivePanelsForParent(secondItemPanel:getId())
	...
	for i, v in ipairs(data) do
		for z, donor in ipairs(v.donors) do
			...
			itemWidget.onClick = function()
				...
				Forge:setWidget(requiredTransferCoreCount, coreCost)
				Forge:updateWidget("core", requiredTransferCoreCount, coreCost)
				local cost = Forge.data.convergenceTransfer[tostring(tier)]
				Forge:setWidget(requiredTransferMoney, cost)
				Forge:updateWidget('money', requiredTransferMoney, cost)
