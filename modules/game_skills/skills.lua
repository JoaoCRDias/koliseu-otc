...
local function applyOutfitBonusDisplay(data)
    ...
    local function setWidgetVisible(id, vis)
        local w = skillsWindow:recursiveGetChildById(id)
        if w then w:setVisible(vis) end
    end

    local function renderDetailList(listId, scrollbarId, headerId, infoId, details, equippedEntry, isMount)
        local list = skillsWindow:recursiveGetChildById(listId)
        if not list then return end
        list:destroyChildren()

        local hasDetails = details and #details > 0
        setWidgetVisible("separator" .. (isMount and "Mount" or "Outfit") .. "List", hasDetails)
        setWidgetVisible(headerId, hasDetails)
        setWidgetVisible(infoId, hasDetails)
        setWidgetVisible(listId, hasDetails)
        setWidgetVisible(scrollbarId, hasDetails)

        if not hasDetails then return end

        local hLabel = skillsWindow:recursiveGetChildById(headerId)
        if hLabel then
            local owned = isMount and mountsOwned or outfitsOwned
            local total = isMount and mountsTotal or outfitsTotal
            hLabel:setText(string.format("%s (%d/%d desbloqueadas)", isMount and "MONTARIAS" or "OUTFITS", owned, total))
        end

        if equippedEntry then
            local iLabel = skillsWindow:recursiveGetChildById(infoId)
            if iLabel then
                local tag = isMount and "Montada" or "Outfit equipada"
                iLabel:setText(tag .. ": " .. equippedEntry.name .. " (" .. (equippedEntry.bonusText or "") .. ")")
            end
        else
            local iLabel = skillsWindow:recursiveGetChildById(infoId)
            if iLabel then
                iLabel:setText(isMount and "Nenhuma montaria ativa" or "Nenhuma outfit com bonus equipada")
            end
        end

        for _, entry in ipairs(details) do
            local row = g_ui.createWidget("SmallSkillButton", list)
            row:setId("detail_" .. (entry.lookType or entry.mountId or ""))
            local nameL = row:getChildById("name") or row:getChildById("SkillNameLabel")
            local valL = row:getChildById("value") or row:getChildById("SkillValueLabel")
            ...
            if nameL then
                nameL:setText(entry.name .. tag)
                nameL:setColor(entry.owned and "#44AD25" or "#666666")
            end
            if valL then
                valL:setText(entry.bonusText or "")
                valL:setColor(entry.owned and "#44AD25" or "#666666")
            end

            local tooltip = entry.name .. "\n"
            if entry.passive then
                tooltip = tooltip .. "Type: Passive\n"
            else
                tooltip = tooltip .. "Type: Equipped\n"
            end
            tooltip = tooltip .. (entry.owned and "Status: Owned" or "Status: Not owned")
            if entry.bonusText and entry.bonusText ~= "" then
                tooltip = tooltip .. "\n\nBonus:\n"
                for part in string.gmatch(entry.bonusText, "[^,]+") do
                    tooltip = tooltip .. "  " .. part:match("^%s*(.-)%s*$") .. "\n"
                end
            end
    ...
    local equippedMount = nil
    if data.outfitDetails then
        for _, e in ipairs(data.outfitDetails) do
            if e.equipped then equippedOutfit = e break end
        end
    end
    if data.mountDetails then
        for _, e in ipairs(data.mountDetails) do
            if e.mounted then equippedMount = e break end
        end
    end

    renderDetailList("outfitBonusList", "outfitBonusScrollbar", "labelOutfitListHeader", "labelEquippedInfo", data.outfitDetails, equippedOutfit, false)
    renderDetailList("mountBonusList", "mountBonusScrollbar", "labelMountListHeader", "labelMountedInfo", data.mountDetails, equippedMount, true)
