...
local function getRuleSummary(rule)
  ...
  if rule.type == "spell" and isMonkVocation() then
  ...
  -- R: Ranged monster names indicator (only for specific spells with rangedMonsterNames set)
  if rule.type == "spell" and isRangedMonsterSpell(rule.spellId) and rule.rangedMonsterNames and rule.rangedMonsterNames ~= "" then
...
-- Parse the ignore monster list into a table of lowercase monster names
function magicShooter.getIgnoreMonsterTable()
  ...
  local ignoreTable = {}
  local text = helperConfig.ignoreMonsterList or ""
